...
if !hp <= 0 && !global.trailermode && !(room == room_editor && global.play == 0) && !instance_exists(obj_pausemenu) && room != hub_1 && room != rm_changelog
{
	...
	draw_set_alpha(1); //v124,158
	draw_healthbar(19, 43, 310, 158, (movespeed / 16) * 100, c_white, c_white, c_white, 0, false, false);
	gpu_set_blendenable(true);
	gpu_set_colorwriteenable(true,true,true,true);
	
	gpu_set_blendmode_ext(bm_dest_alpha,bm_inv_dest_alpha);
	gpu_set_alphatestenable(true);
	draw_sprite(spr_speedbar_fill, 0, 192, 45); //0g 128
	gpu_set_alphatestenable(false);
	gpu_set_blendmode(bm_normal);
	
	draw_sprite(spr_speedbar_bar, 0, 192, 45) //og 128
	...
	draw_sprite(_sprite, noiseicon_speed, 32 + _noiseicon_x, 38)
	draw_sprite(spr_teacup, 0, 128+42, 80)
	...
	draw_text(150+20, 110, string(global.collect) + "/20");
