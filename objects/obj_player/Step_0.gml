...
if (state == states.jump) //eugh shit jumpfix
{
...
if input_buffer_jump > 0
	input_buffer_jump--;
if (!place_meeting(x,y+1,obj_solid)) && (keyboard_check_released(ord("Z"))) && (!candivedash) && (state != states.airdash)// shit
{
	vsp += 1
	candivedash = 1
}
...
{
	if (keyboard_check_pressed(key_c))
	{
		airdashing = 1
...
switch state
{
	case states.groundpound:
		...
		}else
		{
			didpound = 0
			vsp = 0
			image_angle += (360-image_angle) / 7
			if (round(image_angle) >= 355)
			{
				vsp = 5
				image_angle = 0
				endedpound = 1
		...
		if (grounded)
		{
			...
			obj_camera.shakestrength = 5
			if (obj_camera.shake == 0)
			{
				state = states.normal
				endedpound = 0
			}
	...
	case states.airdash:
		...
		{
			vsp -= 0.5
			if (!keyboard_check(vk_left)) && (!key_right)/* || (!key_left)*/
			{
				if (key_up) || (key_down)
				{
					movespeed = Approach(movespeed,0,4)
				}
			}else
			{
				movespeed = Approach(movespeed,dashspeed,4)
				if (!key_up) && (!key_down)
				{
			...
			if (key_up) 
			{
				vsp = Approach(vsp, dashspeed * -1,3)
			}
			if (key_down)
			{
				vsp = Approach(vsp,10,3)
			}
			image_angle = (vsp * (xscale * -1)) * 2.9
		...
		break;
	case states.roll:
		image_speed = movespeed/3.5
		sprite_index = spr_noise_roll
		hsp = xscale * movespeed
		movespeed = Approach(movespeed,0,0.085)
		if (movespeed < 0.10)
		{
			state = states.normal
		}
	...
	case states.normal:
		...
		if (playerstate == 2)
		{
			if (keyboard_check_pressed(ord("X"))) && (instance_number(obj_fireball) < 3)
			{
		...
		if movespeed < 12 //if movespeed below 12 mach2 is not yet
			mach2 = 0;
		...
		else if move != 0 && !place_meeting(x + move, y, obj_solid)
		{
			...
			if (sprite_index != spr_noisewalk) //useless, but eugh
			{
				if (!keyboard_check(key_run))
				{
					image_index = 0;
					sprite_index = spr_noisewalk
				}
			}
			...
			if mach2 < mach2_time && movespeed <= 11.9
			{
				if (!keyboard_check(key_run))
				{
					if movespeed < 6
					...
					{	
						...
						{
							movespeed = Approach(movespeed,6,5)
						}
						mach2++;
					}
				}else
			...
			else
			{
				...
				else
				{
					...
					{
						if movespeed < 19 //speed increasing into mach2
						{
							if character == 1
							{
		...
		if input_buffer_jump && !airdashing
		{
			input_buffer_jump = 0;
			...
			{
				sound_play_3d(sfx_jump, x, y, random_range(0.8,1.3));
				image_index = 0
			...
			else if mach2 >= mach2_time
			{
				if (character == 0)
				{
					...
					vsp = -18;
					//junk beach skip pawsible? //no it isnt, i nerfed this shit
					jumpclouds = 10;
				}
				else
				{
		...
		else if key_down2
		{
			if (movespeed < 6.1)
			{
				...
				
				{
					...
					movespeed =0// max(movespeed, 12);
					spins = 0
					hsp = 0
					state = states.crouch;
					image_index = 0;
					sprite_index = spr_noise_crouch;//REPLACE LATER
				}
			}else
			{
				if !((place_meeting(x + sign(hsp)*3, y, obj_solid) or scr_solid_slope(x + sign(hsp)*3, y))
				...
				
				{
					...
					state = states.slide;
					image_index = 0;
					sprite_index = spr_noise_roll;//REPLACE LATER
				}
			}
		}
		break;
	case states.crouch:
		...
		{
			image_speed = Approach(image_speed,spins/3.5,0.3)
			if (move != 0)
			{
				xscale = move 
			}
	...
	case states.slide:
		if sprite_index == spr_noise_roll
		{
			image_speed = movespeed/3.5
		}
		hsp = xscale * movespeed;
...
if state == states.wallslide or (state == states.jump && mach2 >= mach2_time && sprite_index != spr_fall && vsp < 0 && sprite_index != spr_bounce) && (sprite_index != spr_mach2jump) && (sprite_index != spr_mach2fall)
	grav = 0.25;
else
	grav = 0.5;
